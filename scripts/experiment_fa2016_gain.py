"""FA 2016·2017 계약 35건을 더한 것이 예측을 낫게 하는가.
...
출력: output/reports/experiment_fa2016.md
"""
...
BASELINE_FROM = 2018  # 기존 계약 테이블이 담고 있던 가장 이른 FA 연도
...
def main() -> None:
    ...
    for spec in V9.GROUPS:
        ...
        new = time_predictions(spec, df, 2016)
        ...
        print(f"  2016~ 학습   R² {new_score['r2']:.3f}  MAE {new_score['mae']:.2f}억")
        ...
        lines += [
            ...
            f"| 2016년 이후 (확장) | {new_score['r2']:.3f} | {new_score['rmse']:.2f}억 | {new_score['mae']:.2f}억 |",
            "",
        ]

    out = ROOT / "output" / "reports" / "experiment_fa2016.md"
